...
function ChapterList({course, refreshData, edit=true}) {
  return (
    <div className='mt-3'>
        ...
        <div className='mt-2'>
            {course?.courseOutout?.chapters.map((noOfChapters,index)=>(
                ...
                <div className='flex gap-5 items-center'>
                    ...
                    <div>
                        <h2 className='font-medium text-lg'>{noOfChapters?.chapterName} {edit &&<EditChapters course={course} index={index} refreshData={refreshData}/>}</h2>
                        <p className='text-sm text-gray-500'>{noOfChapters?.about}</p>
                        <p className='flex gap-2 text-primary items-center'><HiOutlineClock />{noOfChapters?.duration}</p>
                    </div>
                </div>
